...
add_action('plugins_loaded', function () {
    // Load as early as possible, but after plugins are loaded.
    MMCA_Logger::init();
    MMCA_Admin::init();
}, 1);

/**
 * Capture fatal errors and notify immediately.
 * Note: Fatal errors can prevent wp-admin access. This plugin logs to DB and emails a direct link to the log viewer.
 */
add_action('init', function () {
    // Register shutdown handler once WP is bootstrapped.
    register_shutdown_function(function () {
        ...
        if (!empty($settings['only_frontend']) && is_admin()) {
            return;
        }
        ...
        $sig = md5($message . '|' . $file . '|' . $line . '|' . $error['type']);
        ...
        // Always log (even if throttled), so you have a full record.
        $log_id = MMCA_Logger::log(array(
            'signature'  => $sig,
        ...
        $admin_log_url = admin_url('tools.php?page=mm-critical-alerts');
        ...
        $type_label = MMCA_Logger::error_type_label((int)$error['type']);
        ...
        $lines[] = '';
        $lines[] = 'Type: ' . $type_label . ' (' . (int)$error['type'] . ')';
        ...
        $lines[] = 'View logs (wp-admin): ' . $admin_log_url;
        $lines[] = 'Site Health debug: ' . $site_health_url;
        ...
        // If WP_DEBUG_LOG is enabled, hint the path.
